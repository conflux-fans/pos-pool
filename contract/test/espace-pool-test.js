...
describe("EspaceStaking", async function () {
  ...
  let user3;
  let bridge;
  const DEFAULT_BALANCE = ethers.utils.parseEther('10000');
  ...
  it("eSpace pool stage change tests", async function () {
    ...
    const beforeBalance = await ethers.provider.getBalance(bridge.address);
    expect(beforeBalance).to.equal(ethers.utils.parseEther('10010.0'));
    ...
    expect(newBalance).to.equal(poolBalance.add(ethers.utils.parseEther('5')));
    ...
    poolBalance = await ethers.provider.getBalance(pool.address);
    expect(poolBalance).to.equal(newBalance.sub(ethers.utils.parseEther('4')));
  ...
  it('eSpace pool userInterest', async function() {
    ...
    expect(user1Interest).to.equal(ethers.utils.parseEther('5'));
    ...
    tx = await pool.connect(bridge).handleUnlockedIncrease(4, {
      value: ethers.utils.parseEther(`${ONE_VOTE_CFX * 4}`),
    });
    await tx.wait();
    ...
    expect(user2Interest).to.equal(ethers.utils.parseEther('5'));
