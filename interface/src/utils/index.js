...
import { CFX_BASE_PER_VOTE,SCAN_URL,
  SCAN_URL_TESTNET,
  SCAN_URL_DEVENT,
  SCAN_URL_POS_DEV,RemainderAmount } from "../constants";
...
export const getCfxByVote=(vote)=>new BigNumber(vote).multipliedBy(CFX_BASE_PER_VOTE).toNumber()
export const getApy=(data)=>new BigNumber(data).dividedBy(100).toNumber()
export const getFee=(userFee)=>new BigNumber(100).minus(new BigNumber(userFee).dividedBy(100)).toNumber()
...
export const getMax=(amount)=>{
  const multiple=Math.floor(new BigNumber(amount).minus(RemainderAmount).dividedBy(CFX_BASE_PER_VOTE).toNumber())
